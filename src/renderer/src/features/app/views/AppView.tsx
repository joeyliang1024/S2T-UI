...
const workspace = view === 'live' ? liveWorkspace : view === 'history' ? (
    ...
    <section className="page-panel settings-panel">
      ...
      <div className="text-service-settings">
        <p className="eyebrow">聲紋註冊</p>
        <div className="model-actions">{voiceprintCaptureState === 'recording' ? <button className="danger" onClick={() => void stopVoiceprintCapture()}>結束錄音</button> : <button className="secondary" disabled={captureState !== 'idle'} onClick={() => void startVoiceprintCapture()}>直接收音錄製</button>}<span>{voiceprintCaptureState === 'recording' ? '錄音中…請持續說話。' : voiceprintFile ? `已準備：${voiceprintFile.name}` : '尚未選擇樣本'}</span></div>
        <label>單一講者 WAV 樣本<input type="file" accept="audio/wav,.wav" onChange={(event) => setVoiceprintFile(event.target.files?.[0] ?? null)} /></label>
