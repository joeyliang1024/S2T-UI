...
const sessionTranscript = (entry: SavedSession, query: string): ReactElement => entry.segments?.length ? <>
    {entry.segments.filter((segment) => segment.status !== 'gap' && `${segment.speaker ?? ''} ${segment.sourceText} ${segment.translatedText ?? ''}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())).map((segment) => <article key={segment.id}>
      ...
      {editingSessionSegmentId === segment.id ? <div className="transcript-edit"><div className="timing-inputs"><label>開始（ms）<input type="number" min="0" defaultValue={segment.startMs} onBlur={(event) => updateSavedTranscriptTiming(entry.id, segment.id, { startMs: Number(event.currentTarget.value) })} /></label><label>結束（ms）<input type="number" min="1" defaultValue={segment.endMs} onBlur={(event) => updateSavedTranscriptTiming(entry.id, segment.id, { endMs: Number(event.currentTarget.value) })} /></label></div><label className="speaker-row"><span>講者</span><input aria-label="講者名稱" value={segment.speaker ?? ''} placeholder="未標記講者" onChange={(event) => updateSessionSpeaker(entry.id, segment.id, event.target.value)} /></label><textarea defaultValue={segment.sourceText} aria-label="原文" onBlur={(event) => updateSavedTranscript(entry.id, segment.id, { sourceText: event.currentTarget.value })} /><textarea defaultValue={segment.translatedText ?? ''} aria-label="翻譯" placeholder="翻譯（選填）" onBlur={(event) => updateSavedTranscript(entry.id, segment.id, { translatedText: event.currentTarget.value })} /><button className="text-button" onClick={() => setEditingSessionSegmentId(null)}>完成編輯</button></div> : <><label className="speaker-row"><span>講者</span><input aria-label="講者名稱" value={segment.speaker ?? ''} placeholder="未標記講者" onChange={(event) => updateSessionSpeaker(entry.id, segment.id, event.target.value)} /></label><p>{segment.sourceText}</p>{segment.translatedText && <p className="translation">{segment.translatedText}</p>}<button className="edit-button" onClick={() => setEditingSessionSegmentId(segment.id)}>編輯</button>{segmentRerecordingId === segment.id ? <button className="text-button" onClick={() => void stopSegmentRerecord()}>結束重講</button> : <button className="text-button" disabled={Boolean(segmentRerecordingId)} onClick={() => void startSegmentRerecord(entry, segment)}>直接重講</button>}<label className="text-button">重講 WAV<input hidden type="file" accept="audio/wav,.wav" onChange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void replaceSessionSegmentAudio(entry, segment, file) }} /></label></>}
    </article>)}
    ...
    <div className="live-workspace">
      <section className="caption-window" style={{ '--caption-scale': captionScale } as CSSProperties}>
        ...
        {visibleTranscripts.length === 0 ? (
          ...
          {settings.translationLoadStrategy === 'manual' && entry.status === 'final' && !entry.translatedText && <button className="text-button translation-retry" onClick={() => void requestTranslation(entry)}>翻譯此段</button>}</article>
        ))}</>}
      </section>
      ...
      <section className="capture-panel capture-controls-only" aria-label="收音控制">
        <div className="source-capture-row"><div className="capture-controls">{canRecord ? <button className="primary" disabled={selectedDeviceId === 'none' && !includeSystemAudio} onClick={() => void startCapture()}>{ui('startRecording')}</button> : captureState === 'starting' || captureState === 'saving' ? <button className="secondary" disabled>{captureState === 'starting' ? '正在連接…' : '正在保存…'}</button> : <><button className="secondary" onClick={() => void togglePause()}>{captureState === 'paused' ? ui('resumeRecording') : ui('pauseRecording')}</button><button className="danger" onClick={() => void stopCapture()}>{ui('stopRecording')}</button></>}</div><div className="timer">{timestamp(elapsedMs)}</div></div>
      </section>
...
const workspace = view === 'live' ? liveWorkspace : view === 'history' ? (
    <section className="page-panel">
      ...
      {sessions.length === 0 ? <div className="empty compact"><h2>{ui('noRecords')}</h2><p>{ui('noRecordsHint')}</p></div> : <>
        ...
        <div className="session-list">{pagedSessions.map((entry) => <article key={entry.id} className="session-item">
          <div className="session-details">{renamingSessionId === entry.id ? <form className="session-rename" onSubmit={(event) => { event.preventDefault(); renameSession(entry.id) }}><input autoFocus aria-label="紀錄標題" maxLength={200} value={titleDraft} onChange={(event) => setTitleDraft(event.target.value)} onBlur={() => renameSession(entry.id)} onKeyDown={(event) => { if (event.key === 'Escape') { setTitleDraft(entry.title); setRenamingSessionId(null) } }} /></form> : <button className="session-title" title="點擊修改標題" onClick={() => { setRenamingSessionId(entry.id); setTitleDraft(entry.title) }}>{entry.title}</button>}<p>{new Date(entry.createdAt).toLocaleString('zh-TW')} · {timestamp(entry.durationMs)} · {entry.source}{entry.audioUnavailable ? ' · 音檔未保存（已下載復原檔）' : entry.savedToDisk ? ' · 已保存' : ' · 尚未保存'}</p>{entry.audioVersions && entry.audioVersions.length > 1 && <label className="session-audio-version">音檔版本<select value={entry.activeAudioVersionId || entry.audioVersions[0].id} onChange={(event) => selectAudioVersion(entry.id, event.target.value)}>{entry.audioVersions.map((version) => <option key={version.id} value={version.id}>{version.label} · {new Date(version.createdAt).toLocaleString('zh-TW')}</option>)}</select></label>}{entry.summary && <><div className="session-summary">{entry.summarySourceSignature && entry.summarySourceSignature !== transcriptSignature(entry.transcript) && <p className="summary-stale">逐字稿已更新，請重新產生摘要。</p>}{renderMarkdown(entry.summary)}</div><div className="session-summary-actions"><button className="text-button" onClick={() => void navigator.clipboard.writeText(entry.summary!)}>複製摘要</button><button className="text-button" onClick={() => downloadSummary(entry.summary!)}>下載 Markdown</button></div></>}{playingSessionId === entry.id && playbackUrl && <audio controls autoPlay src={playbackUrl}>此瀏覽器不支援音訊播放。</audio>}</div>
          <div className="session-actions"><button className="session-more-button" aria-label={`${entry.title} 的更多操作`} aria-expanded={sessionMenuId === entry.id} onClick={() => setSessionMenuId((current) => current === entry.id ? null : entry.id)}>•••</button>{sessionMenuId === entry.id && <div ref={sessionMenuRef} className="session-action-menu" role="menu"><button role="menuitem" onClick={() => { setViewingSessionId(entry.id); setSessionMenuId(null) }}><span>▤</span>查看字幕</button>{!entry.savedToDisk && <button role="menuitem" onClick={() => { void saveSessionToDisk(entry); setSessionMenuId(null) }}><span>↓</span>儲存紀錄</button>}<button role="menuitem" disabled={captureState !== 'idle'} onClick={() => { void continueSession(entry); setSessionMenuId(null) }}><span>↗</span>接續收音</button><button role="menuitem" onClick={() => { setSummarySessionId(entry.id); setView('summary'); setSessionMenuId(null) }}><span>☷</span>產生會議整理</button><button role="menuitem" onClick={() => { void diarizeSession(entry); setSessionMenuId(null) }}><span>◉</span>自動識別講者</button><button role="menuitem" onClick={() => { void playSession(entry); setSessionMenuId(null) }}><span>▶</span>播放錄音</button><button role="menuitem" onClick={() => { void downloadSessionAudio(entry); setSessionMenuId(null) }}><span>↓</span>下載 WAV</button><button role="menuitem" onClick={() => { exportSavedTranscript(entry, 'vtt'); setSessionMenuId(null) }}><span>↓</span>下載 VTT</button><button role="menuitem" onClick={() => { void copyTranscript(entry.segments, copyOptions); setSessionMenuId(null) }}><span>▣</span>複製逐字稿</button><button className="danger" role="menuitem" onClick={() => { void deleteSession(entry); setSessionMenuId(null) }}><span>⌫</span>刪除記錄</button></div>}</div>
        </article>)}</div>
      </>}
    ...
    <section className="page-panel">
      ...
      <div className="model-search"><input aria-label="搜尋模型" value={modelSearch} placeholder="搜尋模型名稱、ID 或 endpoint" onChange={(event) => setModelSearch(event.target.value)} /></div>
    ...
    <section className="page-panel settings-panel">
      ...
      <div className="text-service-settings vad-settings">
        ...
        <label><input type="checkbox" checked={settings.denoiseEnabled} onChange={(event) => setDenoiseEnabled(event.target.checked)} />啟用麥克風降噪</label>
      ...
      {settingsCategory === 'translation' && <><div className="text-service-settings">
        <p className="eyebrow">翻譯 API</p><label><input type="checkbox" checked={settings.translationEnabled} onChange={(event) => setSettings((current) => ({ ...current, translationEnabled: event.target.checked }))} />啟用翻譯</label><label>翻譯策略<select value={settings.translationStrategy} onChange={(event) => setSettings((current) => ({ ...current, translationStrategy: event.target.value as 'realtime' | 'sentence' }))}><option value="realtime">即時逐段</option><option value="sentence">完整句子</option></select></label><label>負載策略<select value={settings.translationLoadStrategy} onChange={(event) => setSettings((current) => ({ ...current, translationLoadStrategy: event.target.value as 'automatic' | 'manual' }))}><option value="automatic">自動排隊</option><option value="manual">手動翻譯</option></select></label>
      ...
      <div className="text-service-settings">
        ...
        <label>搜尋術語<input value={glossarySearch} placeholder="搜尋術語或指定譯法" onChange={(event) => setGlossarySearch(event.target.value)} /></label>
        <div className="glossary-editor">{glossaryEntries.filter((entry) => entry.value.toLocaleLowerCase().includes(glossarySearch.trim().toLocaleLowerCase())).map((entry) => <div key={entry.index}><input defaultValue={entry.value} aria-label="術語" onBlur={(event) => replaceGlossaryEntry(entry.index, event.currentTarget.value)} /><button className="text-button danger" aria-label="刪除術語" onClick={() => replaceGlossaryEntry(entry.index, '')}>刪除</button></div>)}{!glossaryEntries.length && <p className="hint">尚未建立術語。</p>}<div><input value={newGlossaryTerm} aria-label="新增術語" placeholder="例如：Breeze => 指定譯法" onChange={(event) => setNewGlossaryTerm(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addGlossaryEntry() } }} /><button className="secondary" onClick={addGlossaryEntry}>新增術語</button></div></div>
        <label className="glossary-json-import">從 JSON 載入術語<input type="file" accept="application/json,.json" onChange={(event) => { const file = event.currentTarget.files?.[0] ?? null; event.currentTarget.value = ''; void importGlossaryJson(file) }} /></label>
      ...
      {settingsCategory === 'summary' && <div className="text-service-settings summary-template-manager">
        <p className="eyebrow">已保存模板 <span>新增或修改模板請到「摘要」工作區的模板選單選擇「自訂＋」。</span></p>
        <section className="saved-template-list" aria-label="已保存模板"><div>{settings.summaryTemplates.map((template) => <button key={template.id} className={template.id === settings.selectedSummaryTemplateId ? 'active' : ''} onClick={() => selectSummaryTemplate(template.id)}><span>▤</span>{template.name}</button>)}</div></section>
        <div className="summary-template-preview"><span>模板預覽</span><pre>{settings.summaryTemplate}</pre></div>
        <button className="text-button danger" type="button" onClick={deleteSummaryTemplate}>刪除目前模板</button>
      </div>}
