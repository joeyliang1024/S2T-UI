...
const openModelManager = (purpose: 'asr' | 'translation' | 'summary' | 'diarization', id: string | null = null, openedFromModelCard = false): void => {
...
const workspace = view === 'live' ? liveWorkspace : view === 'history' ? (
    ...
    <section className="page-panel">
      ...
      <div className="model-list">
        ...
        {(modelFilter === 'all' || modelFilter === 'translation') && settings.translationProfiles.filter((profile) => matchesModelSearch(profile.name, profile.model, profile.endpoint)).map((profile) => <button className="model-list-item model-translation" key={profile.id} onClick={() => openModelManager('translation', profile.id, true)}><div><strong>{profile.name}</strong><p>翻譯 · OpenAI Chat Completions · {profile.model}</p><code>{textEndpoint(profile.endpoint)}</code></div></button>)}
        ...
        {(modelFilter === 'all' || modelFilter === 'diarization') && settings.diarizationModel && matchesModelSearch(settings.diarizationModel, settings.diarizationEndpoint) && <button className="model-list-item model-diarization" onClick={() => openModelManager('diarization', null, true)}><div><strong>{settings.diarizationModel}</strong><p>講者分離</p><code>{settings.diarizationEndpoint}</code></div></button>}
