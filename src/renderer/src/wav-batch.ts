...
type WavLayout = { sampleRate: number; bytesPerFrame: number; dataOffset: number; dataSizeOffset: number; dataBytes: number; header: Uint8Array }
...
const layoutOf = (input: ArrayBuffer): WavLayout => {
  ...
  while (offset + 8 <= input.byteLength) {
    ...
    if (content + size > input.byteLength) throw new Error('WAV chunk 資料不完整')
    if (id === 'fmt ') {
...
const wavSlice = (input: ArrayBuffer, layout: WavLayout, startByte: number, endByte: number): ArrayBuffer => {
...
export const splitPcmWav = (input: ArrayBuffer, segmentMs = 45_000, overlapMs = 1_500): WavChunk[] => {
  ...
  const segmentBytes = Math.max(layout.bytesPerFrame, Math.floor(segmentMs * bytesPerMs / layout.bytesPerFrame) * layout.bytesPerFrame)
  const overlapBytes = Math.min(segmentBytes - layout.bytesPerFrame, Math.floor(overlapMs * bytesPerMs / layout.bytesPerFrame) * layout.bytesPerFrame)
