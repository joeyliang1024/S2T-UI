...
export const makeVtt = (entries: TranscriptEvent[]): string => `WEBVTT\n\n${entries
  ...
  .map((entry) => {
    const speaker = entry.speaker?.trim() ? `<v ${entry.speaker.trim()}>` : ''
    return `${vttTime(entry.startMs)} --> ${vttTime(entry.endMs)}\n${speaker}${entry.sourceText}${entry.translatedText ? `\n${speaker}${entry.translatedText}` : ''}`
  })
  .join('\n\n')}\n`

export const makeTranscriptText = (entries: TranscriptEvent[], includeSpeaker = false): string => entries
  .filter((entry) => entry.status === 'final')
  .map((entry) => `[${timestamp(entry.startMs)}] ${includeSpeaker && entry.speaker ? `${entry.speaker}：` : ''}${entry.sourceText}${entry.translatedText ? `\n${entry.translatedText}` : ''}`)
  .join('\n\n')
