...
本文件保留早期工程設計、傳輸契約與壓力測試筆記；目前未完成事項與驗收優先順序以 [新版需求單](新版需求單.md) 為準。目標是 Electron 桌面 S2T：錄製麥克風與選用的系統音訊、產生逐字稿與翻譯、保存可重開的工作階段。模型由團隊以 vLLM 或其他服務部署；本專案只負責安全保存設定、送出音訊、接收字幕與保存結果，不內嵌 Python、模型權重或推論服務。

> 程式已由舊的單一 `src/renderer/src/App.tsx` 拆分為 `features/app/views/AppView.tsx`、`features/app/hooks/useAppController.ts` 及各功能 service。以下舊檔案路徑僅保留作為當時設計背景，不可用來判定目前實作狀態。
